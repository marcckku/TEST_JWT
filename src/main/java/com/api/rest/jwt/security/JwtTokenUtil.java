...
//import org.springframework.mobile.device.Device;  //// <<<----- OCCHIO QUI CHE LA PRIMA VOLTA ALL'AVVIO MI HA DATO ERRORE POI HO FIXATO IL POM  E FUNZIONA ADESSO!!.
...
public class JwtTokenUtil implements Serializable {
  ...
  //  @Value("${jwt.secret}") //nel caso non prenda
    private final String secret="mySecret";//////<<<<<<<<   custom here by mpavon

    @Autowired
    ObjectMapper objectMapper;

   // @Value("${jwt.expiration}") ///nel caso non prenda
    private final Long expiration = new Long(7200);  //////<<<<<<<<   custom here by mpavon
   ...
   /* private String generateAudience(Device device) {
    ...
    public String generateToken(UserDetails userDetails) throws JsonProcessingException {//, Device device
       ...
       // claims.put(CLAIM_KEY_AUDIENCE, generateAudience(device));
        ...
        System.err.println("   <<<<<<<<<<< generateToken(claims) >>>>>>><<<   ");
